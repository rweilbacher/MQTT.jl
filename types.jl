mutable struct MQTTMessageInfo
  ...
  function wait_for_publish() end
  function is_published() end
  function set_published() end # maybe needed for notifying other threads

  function MQTTMessageInfo()
    # set defaults
    # implement methods
  end
end
...
mutable struct MQTTMessage
  ...
  mid # message id
  topic # WARNING: this arrives in UTF-8 (I think) so might need to be decoded
  payload
